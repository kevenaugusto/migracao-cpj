...
LEMBRETES = cv2.imread(str(Path(__file__).parent / ASSETS / 'Lembretes.png'), 0)
...
def locate_on_screen(image: cv2.typing.MatLike, screen: MSSBase, max_attempts: int = TENTATIVAS_DE_LOCALIZAR) -> tuple[float, float, float, float]:
        ...
        try:
            while True:
                if busca_sequencial and contador_de_pj > ULTIMO_VALOR_DE_PJ:
                    ...
                    break
                if len(nao_encontradas) >= 100:
                    LOG.debug(f'Pastas Jurídicas NÃO Encontradas: {nao_encontradas}')
                    LOG.debug('Efetuando a Limpeza das Pastas Jurídicas NÃO encontradas.')
                    nao_encontradas = []
                ...
                try:
                    ...
                    pyautogui.press(ENTER)
                try:
                    locate_on_screen(LEMBRETES, default_screen)
                    LOG.debug('Encontrada uma TELA de Lembrete!')
                    found_coord = locate_on_screen(OK, default_screen)
